...
// Type definitions
// -----------------------------------------------------------------------------
// Types live in src/lib/types.ts so other modules can import them without
// pulling in the mock data array. Re-exported here so existing call sites
// that already do `import { MockQuery } from "./mockData"` keep working.
export type { SourceType, MockResult, MockQuery } from "./lib/types";

// Local-only alias so the mockQueries[] literal below stays typeable without
// duplicating the import.
import type { MockQuery } from "./lib/types";
